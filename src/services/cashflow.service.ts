...
import type { CashFlowLine, CashflowAnalyticQueryParams, CashflowQueryParams, CashflowResponse } from '../types';
...
function buildAnalyticQuery(params: CashflowAnalyticQueryParams): string {
  const sp = new URLSearchParams();
...
export async function getCashflowAnalytic(params: CashflowAnalyticQueryParams): Promise<CashFlowLine[]> {
  const qs = buildAnalyticQuery(params);
  return api.get<CashFlowLine[]>(`${endpoints.cashflow.analytic}${qs}`);
}
